...
from .models import Ciudad, RegistroClima, MetricasETL
...
class WeatherstackExtractor:
    def __init__(self):
        ...
        if not self.ciudades or self.ciudades == ['']:
    ...
    def guardar_en_bd(self, datos):
        db = SessionLocal()
        try:
            ciudad_db = db.query(Ciudad).filter_by(nombre=datos['ciudad']).first()
            if not ciudad_db:
                ciudad_db = Ciudad(nombre=datos['ciudad'], pais=datos['pais'])
                db.add(ciudad_db)
                db.commit()
    ...
    def ejecutar_extraccion(self):
        ...
        estado = "SUCCESS" if errores == 0 else ("PARTIAL_SUCCESS" if registros_insertados > 0 else "FAILED")
